...
- [ ] `universal-content-blur.user.js`
  ...
  - [ ] [L] Consider an element-picker quick-add (like the image-resizer) for choosing scope selectors visually, in addition to the keyboard quick-add. Would also be the natural way to capture `class` source patterns (added 2026-07-05) — marker classes aren't selectable/hoverable, so today they must be read from dev tools and typed by hand.
  ...
  - [ ] [M] Hotkey proliferation / leader-key mode (idea, long run): browser, extensions, other scripts and sites already reserve many combos, so every new script hotkey (now Alt+R, Alt+Shift+R, Alt+A, Alt+Shift+S, Alt+Z, Alt+Shift+D, plus peek/commit taps) raises conflict risk. Instead of one combo per action, consider a single global hotkey that enters a "single shortcut key input mode": the next key picks the action (toggle, peek, quick-add, settings, …) and is swallowed, not forwarded to the page, where possible. Closely related to the quick option chooser idea below — could share the key-capture mechanism. Revisit when the next hotkey would be added.
  - [ ] [S] Styles on/off toggle granularity (follow-up to Alt+Z, added 2026-07-05): the toggle is currently global (all effects). If needed later, allow toggling per effect class or per rule — probably not worth its own hotkeys (see leader-key idea above); a small chooser/menu would fit better.
