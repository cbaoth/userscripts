...
`link:invoke-ai-tweaks.user.js[]` adds some tweaks to https://invoke-ai.github.io/InvokeAI/[Invoke-AI] and https://github.com/AUTOMATIC1111/stable-diffusion-webui[Automatic1111] Stable Diffusion WebUI.

* Batch run mode with sampler selection, custom prompts, random substitutions, etc.
* Very basic conversion between Invoke-AI to A1111 notation and vice versa
** E.g. `(foo)++++` to `(foo):1.4` but a) only when using parentheses and b) only for ranges [1.1,1.9] (+) and [0.1,0.9] (-)
* Adds new key bindings:

[cols="1,3",options="header"]
|=======
| Keys         | Action
| alt-F12      | Open batch invocation configuration dialog
| ESC          | Close dialog
|=======
