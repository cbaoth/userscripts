...
* Adds episode number to title (in episode list).
...
| Ctrl-Right  | Fast-forward 10min
| Ctrl-Left   | Rewind 10min
| Ctrl-. _(period)_ | Next video
| Ctrl-, _(comma)_ | Previous video _(playlist only)_
|=======

_Note: All keybindings are disable when an input field (e.g. search or comment) is currently focused, to prevent accidental player actions while typing._
