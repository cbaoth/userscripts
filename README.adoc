...
* Adds rudimentary support for batch processing (invocation sequence with multiple samplers and/or prompt substitutions).
  * Full prompts, or injecting fragments into the original prompt, are supported.
  * The original prompt is reset after the un finishes or is cancelled.
...
==== Ideas
* Add support for random prompt substitutions, like 5 generic textareas listing e.g. artists, colors, styles, city names, etc. which are then randomly injected (one or more at a time) into the prompt via `${random1}` to `${random5}` to make things more fun.
