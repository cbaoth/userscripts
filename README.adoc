...
* Adds rudimentary support for batch processing (invocation sequence with multiple samplers and/or prompt substitutions)
  * Full prompts or partial prompts (appended to original prompt) are supported, original prompt is reset after run finishes / is cancelled.
...
Append partial prompt snippets:
image:invoke-ai-tweaks-batchrun-appendoption.png[]

Wich will result in appending the current snippet to the original prompt:
image:invoke-ai-tweaks-batchrun-appendprompt.png[]
