...
(function () {
    ...
    function genericRateChange(player, up) {
        ...
        if ((up && idx < RATES.length - 1) || (!up && idx > 0)) {
            ...
            rateColor = rate == 1 ? 'white' : rate > 1 ? '#99ff99' : '#ff9999';
            genericShowTT(
                `Speed: <span style="color: ${rateColor}">${rate}x</span></span> already set</i>`,
                'darkgrey',
                '1.5em'
            );
        }
    ...
    function genericPlaybackRateReg(player) {
        ...
        cb.bindKeyDown(
            KEY_BRACKET_LEFT,
            () => {
                genericRateSet(player, 0.25);
            },
            { mods: { shift: true }, skipEditable: true, preventDefault: true }
        );
        // key: = -> playback speed back to default (1)
        cb.bindKeyDown(KEY_EQUAL, () => {
            genericRateSet(player, 1);
        });
    }
    ...
    function amazonTweaksReg() {
        ...
        cb.waitAndThrottle(
            AMAZON_SEL_SKIP,
            (e) => {
                GM_config.get('az-auto-skip') && $(e).click();
            },
            2000,
            { tailing: false }
        );
        cb.waitAndThrottle(
            AMAZON_SEL_SKIP_ADS,
            (e) => {
                GM_config.get('az-auto-skip-ads') && $(e).click();
            },
            2000,
            { tailing: false }
        );
    ...
    function netflixTweaksReg() {
        ...
        // keys: shift+left/+right -> skip +/-1min
        cb.bindKeyDown(KEY_LEFT, (e) => netflixCtrl($('button.button-nfplayerBackTen'), e, 6), {
            mods: { shift: true },
        });
        cb.bindKeyDown(KEY_RIGHT, (e) => netflixCtrl($('button.button-nfplayerFastForward'), e, 6), {
            mods: { shift: true },
        });
        // keys: ctrl+left/+right -> skip +/-10min
        cb.bindKeyDown(KEY_LEFT, (e) => netflixCtrl($('button.button-nfplayerBackTen'), e, 60), {
            mods: { ctrl: true },
        });
        cb.bindKeyDown(KEY_RIGHT, (e) => netflixCtrl($('button.button-nfplayerFastForward'), e, 60), {
            mods: { ctrl: true },
        });
        ...
        cb.waitAndThrottle(
            NETFLIX_SEL_SKIP,
            (e) => {
                GM_config.get('nf-auto-skip') && netflixCtrl(e);
            },
            5000,
            { tailing: false }
        );
    ...
    function youtubeTweaksReg() {
        ...
        function ytQualityChange(up) {
            ...
            if ((up && idx < QUALITY_LEVELS.length - 1) || (!up && idx > 0)) {
                ...
                diffColor = '#ff9999';
                showTT(
                    `Quality: <span style="color: ${diffColor}">${level}</span></span> already set</i>`,
                    'darkgrey',
                    '1.5em'
                );
            }
        ...
        function ytQualitySetIdx(idx = 0) {
            ...
            if (!levelChanged) {
                // unchanged
                showTT(
                    `Quality: <span style="color: ${diffColor}">${newLevel}</span></span> already set</i>`,
                    'darkgrey',
                    '1.5em'
                );
            } else {
                // set new (different) level
                showTT(`Quality: <span style="color: ${diffColor}">${newLevel}</span>`);
        ...
        function ytSetThumb(up, off) {
            ...
            if (off) {
                ...
                // set thumbs up/down flag?
                if (!thumbIsSet) {
                    // flag is not set? then set it
                    showTT(thumbText, thumbColor);
                    button.click();
                } else {
                    // flag is already set? nothing to do
                    showTT(
                        `<i><span style="color: ${thumbColor}">${thumbText}</span> already set</i>`,
                        'darkgrey',
                        '1.5em'
                    );
                }
        ...
        cb.bindKeyDown(
            KEY_RIGHT,
            () => ytplayer.seekTo(Math.min(ytplayer.getCurrentTime() + 60, ytplayer.getDuration() - 1)),
        ...
        cb.bindKeyDown(
            KEY_RIGHT,
            () => ytplayer.seekTo(Math.min(ytplayer.getCurrentTime() + 600, ytplayer.getDuration() - 1)),
        ...
        cb.bindKeyDown(KEY_Q, () => ytQualityChange(true), { skipEditable: true });
        cb.bindKeyDown(KEY_Q, () => ytQualityChange(false), { mods: { shift: true }, skipEditable: true });
        ...
        // skip/close ads
        cb.waitAndThrottle(
            'button.ytp-ad-skip-button, button.ytp-ad-overlay-close-button',
            (e) => {
                $(e).click();
            },
            2000,
    ...
    function disneyPlusTweaksReg() {
        ...
        cb.waitAndThrottle(
            DISNEY_SKIP,
            (e) => {
                GM_config.get('dn-auto-skip') && $(e).click();
            },
            2000,
            { tailing: false }
        );
    ...
    if (/amazon/.test(window.location.host)) {
        ...
        // Netflix
        netflixTweaksReg();
    } else if (/youtube/.test(window.location.host)) {
        // YouTube
        youtubeTweaksReg();
    } else if (/disneyplus/.test(window.location.host)) {
        // Disney+
        disneyPlusTweaksReg();
    } else if (/spotify/.test(window.location.host)) {
        // Spotify
        spotifyTweaksReg();
    } else if (/zdf|3sat/.test(window.location.host)) {
        // ZDF Mediathek
        zdfTweaksReg();
    } else if (/plex/.test(window.location.host)) {
        // PLEX TV
