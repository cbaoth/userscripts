...
(function (cb, $, _, undefined) {
    ...
    cb.nameToInitials = function (
        ...
        if (reverse) {
            return (
                (nameLast !== "" ? nameLast.substr(0, lastLength) + separator : "") +
    ...
    /* {{{ - TOOL-TIP --------------------------------------------------------- */
    var _cbcreateTTStyleInjected = false;
    var _cbcreateTTLastTT;
    ...
    cb.createTT = function (
        ...
        var posLeft = (offsetMouse ? cbMouseX : 0) + offsetX;
        var posTop = (offsetMouse ? cbMouseY : 0) + offsetY;

        // create tt div
        var div = $('<div class="cb_createTT">' + html + "</div>");
        div.css({
            position: "absolute",
            display: "block",
            "z-index": 9999,
