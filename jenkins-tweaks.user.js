...
(function () {
    ...
    const SEL_CONSOLE =
        'pre.console-output,' + // job console selector (static)
        'pre.console-output > pre:not(.cbSubstituteTextWithCSS)'; // active console, auto update
    const SEL_CONSOLE_BO =
        'div.test-console, div.log-body,' + // blue ocean console (static)
    ...
    const CONSOLE_SUBSTITUTION = [
        [
            SEL_CONSOLE + ', ' + SEL_CONSOLE_BO,
            [
                // test progress
    ...
    if (/\/blue\/.*\/(pipeline|tests)(\/|$)/.test(window.location.pathname)) {
        // blue ocean pipeline / test results
        cb.waitAndThrottle(
            SEL_CONSOLE_BO,
            _.debounce(() => subsituteConsoleText(SEL_CONSOLE_BO), 1000)
        );
    } else if (/\/blue\/.*\/log\//.test(window.location.pathname)) {
        // blue ocean plain log file view
        cb.waitAndThrottle(
            SEL_LOG_BO,
            _.debounce(() => subsituteConsoleText(SEL_LOG_BO), 1000)
        );
    } else if (/\/console(Full)?(\/|$)/.test(window.location.pathname)) {
        // job console log
        cb.waitAndThrottle(
            SEL_CONSOLE,
            _.debounce(() => subsituteConsoleText(SEL_CONSOLE), 1000)
        );
    }
    /* {{{ -- CONSOLE HIGHLIGHTING -------------------------------------------- */
})();
